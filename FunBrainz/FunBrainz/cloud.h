#pragma once
#include "GlobalFuncs.h"
#include "cloudgame.h"
#include <stdio.h>  
#include <utility> 
#include<cmath>
#include<ctime>
#include<string>
#include<iostream>
namespace FunBrainz {

	using namespace System;
	using namespace System::ComponentModel;
	using namespace System::Collections;
	using namespace System::Windows::Forms;
	using namespace System::Data;
	using namespace System::Drawing;

	/// <summary>
	/// Summary for cloud
	/// </summary>
	public ref class cloud : public System::Windows::Forms::Form
	{
	public:
		cloud(void)
		{
			InitializeComponent();
			//
			//TODO: Add the constructor code here
			//
		}

	protected:
		/// <summary>
		/// Clean up any resources being used.
		/// </summary>
		~cloud()
		{
			if (components)
			{
				delete components;
			}
		}
	private: System::Windows::Forms::Label^  label1;
	protected: 
	private: System::Windows::Forms::Button^  button1;
	private: System::Windows::Forms::Button^  button2;
	private: System::Windows::Forms::Button^  button3;
	private: System::Windows::Forms::Button^  button4;

	private:
		/// <summary>
		/// Required designer variable.
		/// </summary>
		System::ComponentModel::Container ^components;

#pragma region Windows Form Designer generated code
		/// <summary>
		/// Required method for Designer support - do not modify
		/// the contents of this method with the code editor.
		/// </summary>
		void InitializeComponent(void)
		{
			this->label1 = (gcnew System::Windows::Forms::Label());
			this->button1 = (gcnew System::Windows::Forms::Button());
			this->button2 = (gcnew System::Windows::Forms::Button());
			this->button3 = (gcnew System::Windows::Forms::Button());
			this->button4 = (gcnew System::Windows::Forms::Button());
			this->SuspendLayout();
			// 
			// label1
			// 
			this->label1->AutoSize = true;
			this->label1->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 24, System::Drawing::FontStyle::Bold, System::Drawing::GraphicsUnit::Point, 
				static_cast<System::Byte>(0)));
			this->label1->Location = System::Drawing::Point(257, 71);
			this->label1->Name = L"label1";
			this->label1->Size = System::Drawing::Size(223, 46);
			this->label1->TabIndex = 0;
			this->label1->Text = L"Categories";
			// 
			// button1
			// 
			this->button1->Location = System::Drawing::Point(64, 171);
			this->button1->Name = L"button1";
			this->button1->Size = System::Drawing::Size(186, 75);
			this->button1->TabIndex = 1;
			this->button1->Text = L"Addition";
			this->button1->UseVisualStyleBackColor = true;
			this->button1->Click += gcnew System::EventHandler(this, &cloud::button1_Click);
			// 
			// button2
			// 
			this->button2->Location = System::Drawing::Point(499, 171);
			this->button2->Name = L"button2";
			this->button2->Size = System::Drawing::Size(186, 75);
			this->button2->TabIndex = 2;
			this->button2->Text = L"Subtraction";
			this->button2->UseVisualStyleBackColor = true;
			this->button2->Click += gcnew System::EventHandler(this, &cloud::button2_Click);
			// 
			// button3
			// 
			this->button3->Location = System::Drawing::Point(64, 352);
			this->button3->Name = L"button3";
			this->button3->Size = System::Drawing::Size(186, 75);
			this->button3->TabIndex = 3;
			this->button3->Text = L"Multiplication";
			this->button3->UseVisualStyleBackColor = true;
			this->button3->Click += gcnew System::EventHandler(this, &cloud::button3_Click);
			// 
			// button4
			// 
			this->button4->Location = System::Drawing::Point(499, 352);
			this->button4->Name = L"button4";
			this->button4->Size = System::Drawing::Size(186, 75);
			this->button4->TabIndex = 4;
			this->button4->Text = L"Division";
			this->button4->UseVisualStyleBackColor = true;
			this->button4->Click += gcnew System::EventHandler(this, &cloud::button4_Click);
			// 
			// cloud
			// 
			this->AutoScaleDimensions = System::Drawing::SizeF(8, 16);
			this->AutoScaleMode = System::Windows::Forms::AutoScaleMode::Font;
			this->ClientSize = System::Drawing::Size(753, 526);
			this->Controls->Add(this->button4);
			this->Controls->Add(this->button3);
			this->Controls->Add(this->button2);
			this->Controls->Add(this->button1);
			this->Controls->Add(this->label1);
			this->Name = L"cloud";
			this->Text = L"cloud";
			this->ResumeLayout(false);
			this->PerformLayout();

		}
#pragma endregion

int category;
private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
			 category=0;
			 cloudgame ^ form = gcnew cloudgame(category); 
			 
			 form->ShowDialog();
		 }
private: System::Void button2_Click(System::Object^  sender, System::EventArgs^  e) {
			 category=1;
			 cloudgame ^ form = gcnew cloudgame(category); 
			 
			 form->ShowDialog();
		 }
private: System::Void button3_Click(System::Object^  sender, System::EventArgs^  e) {
			 category=2;
			 cloudgame ^ form = gcnew cloudgame(category);
			 
			 form->ShowDialog();
		 }
private: System::Void button4_Click(System::Object^  sender, System::EventArgs^  e) {
			 category=3;
			 cloudgame ^ form = gcnew cloudgame(category); 
			 form->ShowDialog();
		 }
};
}
